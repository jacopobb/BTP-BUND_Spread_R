...
data  <- read.csv("group_work/spread.csv")
data  <- xts(data[, -1], order.by = as.Date(data$date))
head(data)

spread <- na.omit(data$spread)
ls  <- na.omit(log(spread))
dls <- na.omit(diff(ls))

# Split data into train/test
n_test  <- round(length(dls) * 0.2)
n_train <- length(dls) - n_test

train <- dls[1:n_train]
test  <- dls[(n_train + 1):length(dls)]
...
# DLS
plot(index(dls), coredata(dls), type = "l", lwd = 0.7, 
     main = "Differenced log spread (DLS)", xlab = "", ylab = "dls")
abline(h = 0, col = "black", lwd = 0.5)

# ACF
Acf(train, lag.max = 30, main = "ACF of DLS")

# PACF 
Pacf(train, lag.max = 30, main = "PACF of DLS")

# ACF of squared dls (volatility clustering)
Acf(train^2, lag.max = 30, main = "ACF of Squared DLS")
...
# Extract the density predictions and realized dls
...
# Use dls^2 as a proxy for realized volatility
...
p_dls  <- ggplot(forecast_df, aes(x = date)) +
  ...
  labs(title = "DLS: Actual vs Forecasted", x = "", y = "DLS") +
  scale_x_date(date_breaks = "1 year", date_label = "%Y") +
  theme_minimal() +
...
get_spreads <- function() {
  ...
  for (i in 1:n_test) {
    ...
    prev_spread <- as.numeric(ls[n_train + i - 1]) 
    
    fc <- forecast_df$pred_mu[i] 
    fc_spread[i] <- exp(prev_spread + fc)
    
    ci_lower <- forecast_df$ci_q05[i] 
    ci_upper <- forecast_df$ci_q95[i] 
    
    lower_spread[i] <- exp(prev_spread + ci_lower)
    upper_spread[i] <- exp(prev_spread + ci_upper)
  }
...
spread_conversion <- function(value, q_05, q_95, data){
	...
	spread <- exp(last_spread + value)
	q_05 <- exp(last_spread + q_05)
	q_95 <- exp(last_spread + q_95)
...
future_spread <- spread_conversion(future_fc$dls, future_fc$q_05, future_fc$q_95, ls)

cat("\nForecasted values:\n")

cat("\nDLS on last date:", as.character(last_date), ": ", tail(dls, 1))
cat("\nDLS forecasted for next week:", as.character(last_date + 7), ":\n")
print(future_fc)
...
cat("\nDLS simulated over 10.000 paths for next week", as.character(last_date + 7), ":\n")
cat("\nMean DLA: ", mean)
cat("\nMean Vol:", vol)
cat("\nQuantiles 5% | 95%: ", q_90, "\n")

# Conversion to spread
sim_spread <- spread_conversion(value = mean, q_05 = unname(q_90[1]), q_95 = unname(q_90[2]), data = ls)
